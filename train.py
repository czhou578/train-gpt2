...
sample_top_k = 200
metrics_log_file = 'metrics_wikitext_128.jsonl' # structured, machine-parseable log (one JSON object per line)
train_log_file = 'train_wikitext_128.log' # human-readable log, mirrors stdout
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(out_dir, train_log_file)),
...
def log_metrics(record: dict):
    """Append one JSON record to metrics.jsonl for later analysis/plotting."""
    record = {'timestamp': time.time(), **record}
    with open(metrics_path, 'a') as f:
        f.write(json.dumps(record) + '\n')

def get_gpu_memory():
    if device_type != "cuda": return {}

    return {
        'mem_allocated_gb': torch.cuda.memory_allocated() / 1e9,
        'mem_reserved_gb': torch.cuda.memory_reserved() / 1e9,
        'max_mem_allocated_gb': torch.cuda.max_memory_allocated() / 1e9,
        'max_mem_reserved_gb': torch.cuda.max_memory_reserved() / 1e9,
    }

def get_gpu_util():
    """Raw GPU utilization %, distinct from MFU (which is compute efficiency vs. peak FLOPs).
    Requires nvidia-smi on PATH; returns None if unavailable."""
    if device_type != 'cuda':
        return None
    try:
...
# log run-level metadata once, so multiple runs (different model sizes) can be diffed later
# from metrics.jsonl to look at scaling behavior
...
log_metrics({'event': 'run_start', 'config': config, 'num_params': num_params})
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        if val_loss < best_val_loss or always_save_checkpoint:
            ...
            if iter_num > 0:
                ...
                log_metrics({'event': 'checkpoint', 'iter': iter_num, 'best_val_loss': best_val_loss.item()})
